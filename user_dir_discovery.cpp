...
/*
-------------------------------------------
Now the socket and its outgoing network TCP connection are established; 
data can now be sent over TCP. In this case, the server variable holds the connection details, 
so further sending any data to this target will be done using the server variable.
...
directory, as long as there are next items: */
    ...
    else 
    {
        ...
        print the error - this can be deleted from the final version if it works correctly so that the target does not see errors printed */
    }
